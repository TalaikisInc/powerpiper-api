...
import { IFactoryInterface } from './FactoryInterface'
import { EntityFactory } from './EntityFactory'
import { BluePrint } from './BluePrint'
import { ISeedsConstructorInterface } from './SeedsInterface'

export class Factory implements IFactoryInterface {
    ...
    public async runSeed<T>(seedClass: ISeedsConstructorInterface): Promise<T> {
        const seeder = new seedClass()
        return await seeder.seed(this)
    }

    public define<Entity>(entityClass: ObjectType<Entity>, callback: (faker: typeof Faker, args: any[]) => 
      Entity): void {
